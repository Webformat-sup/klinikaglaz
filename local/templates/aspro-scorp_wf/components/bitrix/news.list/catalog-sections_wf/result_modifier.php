...
$arResult['SECTIONS'] = $CCache->CIBLockSection_GetList(array('SORT' => 'ASC', 'CACHE' => array('TAG' => $CCache->GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), $arSectionsFilter, false, array('ID', 'NAME', 'IBLOCK_ID', 'DEPTH_LEVEL', 'SECTION_PAGE_URL', 'PICTURE', 'DETAIL_PICTURE', 'UF_INFOTEXT', 'DESCRIPTION'));
	...
	foreach($arResult['SECTIONS'] as $key => $arSection)
	{
		...
		foreach($arResult['ITEMS'] as $arItem){
			if($key == $arItem['IBLOCK_SECTION_ID']){
				$arResult['SECTIONS'][$key]['ITEMS'][$arItem['ID']] = $arItem;
			}
		}		
