...
<div class="bx_filter <?=$templateData["TEMPLATE_CLASS"]?> bx_horizontal">
	<div class="bx_filter_section">
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter" style="display: -webkit-flex;    -webkit-flex-flow: column wrap;   display: flex;    flex-flow: column wrap;">
			<div class="row" style="order: 1; -webkit-order: 1;">
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					<div class="filter-item">
						<div class="bx_filter_parameters_box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>checked_parameter <?endif?>">
							<span class="bx_filter_container_modef"></span>
				...
				<div class="filter-item item-buttons">	
					<div class="bx_filter_button_box active">
						<div class="bx_filter_block"style=" padding-top: 6px;">
							<div class="bx_filter_parameters_box_container" >
...
<?if($result){?>
	<div class="row filter-footer" >
		<span style="text-align: center;">Вы выбрали: <?
		foreach($result as $i => $r){ 
			echo $r;
			if(count($result)>1 && $i<(count($result)-1)) echo ', ';
		} 
		?></span>
		<span style="text-align: center;">(<a href="<?=$APPLICATION->GetCurPage();?>">вернуться к полному списку отзывов</a>)</span>
	</div>
<?}?>
		...
		$(".bx_filter_parameters_box").each(function(index, element){
			...
			if ($(element).is(e.target) || $(element).has(e.target).length > 0){
				// console.log(e.target);
				 if($(e.target).closest(".bx_filter_block").length > 0){
					return;
				 }
					$(element).find(".bx_filter_block").slideToggle(100);
					$(element).toggleClass("active");
			...
			$.cookie.json = true;
			$.cookie("NEXT_filter_prop_"+$(element).data('prop_code'), active,{
				path: '/',
				domain: '',
				expires: 360
			});
