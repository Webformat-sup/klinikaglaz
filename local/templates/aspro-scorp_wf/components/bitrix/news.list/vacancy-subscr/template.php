						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?if($arParams['SHOW_SECTION_PREVIEW_DESCRIPTION'] == 'Y'):?>
								...
								<?if(strlen($arSection['NAME'])):?>
									<h3 class="underline" id="<?=$arSection['CODE']?>"><?=$arSection['NAME']?></h3>
								<?endif;?>
							...
							<?foreach($arSection['ITEMS'] as $i => $arItem):?>
								...
								<?ob_start();?>
									...
									<?endif;?>
									<button class="btn btn-default" data-event="jqm" data-name="resume" data-param-id="23" data-autoload-STAFF="<?=$arItem['NAME']?>" data-autohide=""><?=GetMessage('BUTTON')?></button>
