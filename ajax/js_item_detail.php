...
<?if($arPost["PARAMS"]):?>
	...
	else
	{
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			$arItems["ITEMS"][$key] = array(
				...
				"NAME" => $arItem["NAME"],
				"IBLOCK_ID" => $arPost["PARAMS"]["MAIN_IBLOCK_ID"],
				...
				"PROPERTIES" => [$arPost["PARAMS"]["OFFER_ADD_PICT_PROP"] => $arItem["PROPERTIES"][$arPost["PARAMS"]["OFFER_ADD_PICT_PROP"]]],
	...
	if($arItems)
	{
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			if($arPost["PARAMS"]["GALLERY_ITEM_SHOW"] == "Y")
			{
				if (!$arItem["PREVIEW_PICTURE_FIELD"]) {
					if ($arItem["DETAIL_PICTURE"]) {
						$arItem["PREVIEW_PICTURE_FIELD"] = $arItem["DETAIL_PICTURE"];
					}
				} else {
					$arItem["DETAIL_PICTURE"] = $arItem["PREVIEW_PICTURE_FIELD"];
				}
				
				$arItems["ITEMS"][$key]["GALLERY"] = CNext::getSliderForItemExt($arItem, $arPost["PARAMS"]["OFFER_ADD_PICT_PROP"], true);

				// if(!$arItems["ITEMS"][$key]["GALLERY"] && $arItems['MAIN_ITEM'])
				if($arItems['MAIN_ITEM']/* && 'Y' == $arPost["PARAMS"]['ADD_DETAIL_TO_GALLERY_IN_LIST']*/)
				{
					$arItems["ITEMS"][$key]["GALLERY"] = array_merge($arItems["ITEMS"][$key]["GALLERY"], CNext::getSliderForItemExt($arItems['MAIN_ITEM'], $arPost["PARAMS"]["ADD_PICT_PROP"], 'Y' == $arPost["PARAMS"]['ADD_DETAIL_TO_GALLERY_IN_LIST'], true, true));
				}

				array_splice($arItems["ITEMS"][$key]["GALLERY"], $arPost["PARAMS"]["MAX_GALLERY_ITEMS"]);
				$arItems["ITEMS"][$key]["GALLERY_HTML"] = \Aspro\Functions\CAsproNext::showSectionGallery( array('ITEM' => $arItems["ITEMS"][$key], 'RETURN' => true) );
			}
	...
	<script>
		...
		/*set store quantity*/
		setQuantityStore = function(ob, text, indicators)
		{
			if(parseFloat(ob.MAX_QUANTITY)>0){
				wrapper.find('.item-stock .icon').removeClass('order stock_range_1 stock_range_2 stock_range_3').addClass('stock stock_range_' + indicators);
			}
			else{
				wrapper.find('.item-stock .icon').removeClass('stock stock_range_1 stock_range_2 stock_range_3').addClass('order');
			}
		...
		ChangeInfo = function()
		{
			...
			if(-1 < index)
			{
				...
				if('GALLERY' in obOffers[index] && obOffers[index].GALLERY.length)
				{
					//console.log(obOffers[index].GALLERY)
					obOffers[index].GALLERY_HTML = $(obOffers[index].GALLERY_HTML).children();
					wrapper.find('.image_wrapper_block .thumb').removeClass('shine').html(obOffers[index].GALLERY_HTML);
					// InitLazyLoad();
				}
				else
				{
					if(!!obOffers[index].PICTURE) {
					//console.log(obOffers[index].PICTURE)
				...
				setQuantityStore(obOffers[index], obOffers[index].AVAILIABLE.TEXT, obOffers[index].AVAILIABLE.INDICATORS);
				...
				<?//if($arPost['PARAMS']['SHOW_PROPS'] == 'Y'):?>
					if(wrapper.find('.props_list.offers').length)
					{
						var props = '';
						...
						wrapper.find('.props_list.offers').html(props).show();
					}
				<?//endif;?>
		...
		for (i = 0; i < depth; i++)
		{
			...
			if (selectedValues[strName] !== undefined) {
