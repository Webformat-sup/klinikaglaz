...
file_put_contents('/home/bitrix/ext_www/klinikaglaz.ru/LOG.txt', print_r(['test2'], true), FILE_APPEND);
...
while($arFields = $res->GetNextElement())
{
		...
		$arElement[$u]['AGE_CATEGORY'] = $arProps['AGE_CATEGORY']['VALUE'];
		$arElement[$u]['ACADEMIC_DEGREE'] = $arProps['ACADEMIC_DEGREE']['VALUE'];
	...
	{  
		...
		// не у всех врачей заполнен Опыт работы (обязательное поле)
				...
				$xmlWriter->startElement('offer');
					...
					if(!empty($element['AGE_CATEGORY']))
					{
						if(in_array('Взрослый врач', $element['AGE_CATEGORY']))
						{
							$xmlWriter->startElement('param');
								$xmlWriter->writeAttribute('name', 'Взрослый врач');
								$xmlWriter->text('true');
							$xmlWriter->endElement();
						}
						if(in_array('Детский врач', $element['AGE_CATEGORY']))
						{
							$xmlWriter->startElement('param');
								$xmlWriter->writeAttribute('name', 'Детский врач');
								$xmlWriter->text('true');
							$xmlWriter->endElement();
						}
					}
					
					if(!empty($element['ACADEMIC_DEGREE']))
					{
						$xmlWriter->startElement('param');
							$xmlWriter->writeAttribute('name', 'Ученая степень');
							$xmlWriter->text($element['ACADEMIC_DEGREE']);
						$xmlWriter->endElement();
					}
