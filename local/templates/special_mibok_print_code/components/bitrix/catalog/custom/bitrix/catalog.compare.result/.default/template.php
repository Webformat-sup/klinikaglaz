...
?><div class="bx_sort_container">
	<h3 class="sorttext"><?=GetMessage("CATALOG_SHOWN_CHARACTERISTICS")?>:</h3>
	<a class="sortbutton btn btn-default <? echo (!$arResult["DIFFERENT"] ? ' current' : ''); ?>" href="<? echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=N'; ?>" rel="nofollow"><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?></a>
	<a class="sortbutton btn btn-default<? echo ($arResult["DIFFERENT"] ? ' current' : ''); ?>" href="<? echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=Y'; ?>" rel="nofollow"><?=GetMessage("CATALOG_ONLY_DIFFERENT")?></a>
</div>
<?
...
<div class="bx_filtren_container">
	<h4><?=GetMessage("CATALOG_COMPARE_PARAMS")?></h4>
	<ul><?
...
if (!empty($arResult["SHOW_FIELDS"]))
{
	...
	{
		$showRow = true;
		if ((!isset($arResult['FIELDS_REQUIRED'][$code]) || $arResult['DIFFERENT']) && is_array($arResult["ITEMS"]) && count($arResult["ITEMS"]) > 1)
		{
			$arCompare = array();
		...
		<?
				switch($code)
				{
					case "NAME":
						...
						<noindex><br /><a class="btn btn-default" href="<?=$arElement["BUY_URL"]?>" rel="nofollow"><?=GetMessage("CATALOG_COMPARE_BUY"); ?></a></noindex>
...
if (!empty($arResult["SHOW_OFFER_PROPERTIES"]))
{
	...
	<tr>
		...
		?>
		<td>
            <a onclick="CatalogCompareObj.delete('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>');" href="javascript:void(0)" class="btn btn-default btn-sm"><?=GetMessage("CATALOG_REMOVE_PRODUCT")?></a>
		</td>
		<?
		}
		unset($arElement);
