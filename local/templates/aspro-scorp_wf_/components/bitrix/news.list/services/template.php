		...
		<div class="items row">
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?ob_start();?>
					...
					<div class="prevorder">
								<span class="btn btn-default btn-sm" data-event="jqm" data-param-id="<?=CCache::$arIBlocks[SITE_ID]['aspro_scorp_form']['aspro_scorp_order_services'][0]?>" data-name="order_services" data-autoload-service="<?=$arItem['NAME']?>"><span><?=(strlen($arParams['S_ORDER_SERVICE']) ? $arParams['S_ORDER_SERVICE'] : GetMessage('S_ORDER_SERVICE'))?></span></span>
					</div>
				<?e/*ndif;*/?>
					...
					<div class="item <?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<div class="row">
							...
							<?elseif($arParams['IMAGE_POSITION'] == 'right'):?>
								<div class=""><div class="text"><?=$textPart?></div></div>
								<div class="item "><?=$imagePart?></div>
							<?else:?>
								<div class=""><?=$imagePart?></div>
								<div class="item "><div class="text"><?=$textPart?></div></div>
							<?endif;?>
						</div>
					</div>
				</div>
