		...
		<div class="items row">
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?ob_start();?>
					...
					<div class="prevorder">
								<span class="btn btn-default btn-sm" data-event="jqm" data-param-form_id="SERVICES" data-name="order_services" data-autoload-service="<?=$arItem['NAME']?>"><span><?=(strlen($arParams['S_ORDER_SERVICE']) ? $arParams['S_ORDER_SERVICE'] : GetMessage('S_ORDER_SERVICE'))?></span></span>
					</div>
				<?e/*ndif;*/?>
					...
					<div class="item <?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<div class="row">
							...
							<?elseif($arParams['IMAGE_POSITION'] == 'right'):?>
								<div class="itemtext"><div class="text"><?=$textPart?></div></div>
								<div><?=$imagePart?></div>
							<?else:?>
								<div><?=$imagePart?></div>
								<div class="itemtext"><div class="text"><?=$textPart?></div></div>
