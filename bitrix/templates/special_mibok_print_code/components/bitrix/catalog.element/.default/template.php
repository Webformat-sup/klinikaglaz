										...
										<div class="product-item-detail-info-container">
											...
											if ($arParams['SHOW_OLD_PRICE'] === 'Y')
											{
												...
												<div class="item_economy_price" id="<?=$itemIds['DISCOUNT_PRICE_ID']?>"
													style="display: <?=($showDiscount ? 'inline-block' : 'none')?>;">
													<?
													if ($showDiscount)
													{
                                                ...
                                                <?if ($arParams['SHOW_DISCOUNT_PERCENT'] === 'Y')
                                                {
                                                    if ($haveOffers)
                                                    {
                                                        ...
                                                        (<span class="product-item-label-ring1 " id="<?=$itemIds['DISCOUNT_PERCENT_ID']?>"
                                                            style="display: none;">
                                                        </span>)
                                                        <?
                                                    }
                                                    else
                                                    {
                                                        if ($price['DISCOUNT'] > 0)
                                                        {
                                                            ...
                                                            (<span class="product-item-label-ring1 " id="<?=$itemIds['DISCOUNT_PERCENT_ID']?>"
                                                                title="<?=-$price['PERCENT']?>%">
                                                                <strong><?= GetMessage('MIBOK_CATALOG_DISCOUNT')?></strong><?=-$price['PERCENT']?>%</span>
                                                            </span>)
                                                            <?
                                                        }
                                                    }
											...
											<div class="product-item-detail-info-container" style="<?=(!$actualItem['CAN_BUY'] ? 'display: none;' : '')?>"
												...
												<div class="product-item-amount">
													<div class="product-item-amount-field-container">
														<span class="product-item-amount-field-btn-minus btn btn-default btn-count no-select" id="<?=$itemIds['QUANTITY_DOWN_ID']?>">-</span>
														<input class="product-item-amount-field" id="<?=$itemIds['QUANTITY_ID']?>" type="number"
															value="<?=$price['MIN_QUANTITY']?>">
														<span class="product-item-amount-field-btn-plus btn btn-default btn-count no-select" id="<?=$itemIds['QUANTITY_UP_ID']?>">+</span>
										...
										<div data-entity="main-button-container">
											<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
												...
												if ($showAddBtn)
												{
													?>
													<div class="product-item-detail-info-container">
														<a class="btn <?=$showButtonClassName?> " id="<?=$itemIds['ADD_BASKET_LINK']?>"
												...
												if ($showBuyBtn)
												{
													?>
													<div class="product-item-detail-info-container">
														<a class="btn <?=$buyButtonClassName?> " id="<?=$itemIds['BUY_LINK']?>"
															href="javascript:void(0);">
											...
											?>
											<div class="product-item-detail-info-container">
												<a class="btn btn-link " id="<?=$itemIds['NOT_AVAILABLE_MESS']?>"
													href="javascript:void(0)"
							...
							<ul class="nav nav-tabs">
								...
								/*if ($arParams['USE_COMMENTS'] === 'Y')
								{
									...
									<li class="product-item-detail-tab" data-entity="tab" data-value="comments">
										<a href="#comments" class="" data-toggle="tab">
