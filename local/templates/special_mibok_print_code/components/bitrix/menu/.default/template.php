...
if($arParams['ROOT_MENU_TYPE'] == 'top'){
    $arClasses[] = 'top-menu';
    $label =GetMessage("MIBOK_SP_MENU_TOP_TITLE");
}else if($arParams['ROOT_MENU_TYPE'] == 'bottom'){
    $arClasses[] = 'bottom-menu';
    $label =GetMessage("MIBOK_SP_MENU_BOTTOM_TITLE");
}else if($arParams['ROOT_MENU_TYPE'] == 'glazamibok'){
    $arClasses[] = 'special-mibok-menu';
    $label =GetMessage("MIBOK_SP_MENU_SPECIAL_TITLE");
}else if($arParams['ROOT_MENU_TYPE'] == 'left'){
    $arClasses[] = 'left-menu';
    $label =GetMessage("MIBOK_SP_MENU_LEFT_TITLE");
}else{
    $arClasses[] = 'mibok-menu';
    if($arParams['TITLE']){
        $label = $arParams['TITLE'];
    }else{
        $label =GetMessage("MIBOK_SP_MENU_SERVICE_TITLE");
    }    
}
...
<nav class="navbar navbar-default <?=implode(' ', $arClasses)?>">
    <div class="container-fluid">
        <div class="collapse1 " id="bs-navbar-<?=$menu_id?>">
...
<?if ($arItem["IS_PARENT"]):
	...
	<?if ($arItem["DEPTH_LEVEL"] == 1):?>
                ...
                <li role="menuitem" aria-haspopup="true" class="dropdown <?if ($arItem["SELECTED"]):?>active<?endif?>" tabindex="<?=($firstRoot ? '-1' : '0')?>" aria-label="<?=GetMessage("MENU_ITEM_LABEL")?> <?=$arItem["TEXT"]?>">
                    <a href="<?=$arItem["LINK"]?>" class="dropdown-toggle" role="button" aria-expanded="false" tabindex="-1"><?=$arItem["TEXT"]?><span class="caret"></span></a>			
