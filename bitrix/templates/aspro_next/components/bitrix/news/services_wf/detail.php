...
$arElement = CCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N')), $arItemFilter, false, false, array('ID', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'DETAIL_PAGE_URL', 'LIST_PAGE_URL', 'PROPERTY_LINK_PROJECTS', 'PROPERTY_LINK_GOODS', 'PROPERTY_LINK_REVIEWS', 'PROPERTY_LINK_STAFF', 'PROPERTY_LINK_AKCII'));
?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?>">
		...
		<?if(in_array('LINK_STAFF', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_STAFF_VALUE']):?>
			<div class="wraps nomargin">
				...
				<?$APPLICATION->IncludeComponent("bitrix:news.list", "staff-linked_custom", array(
		...
		<?// services links?>
		<?if(in_array('LINK_AKCII', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_AKCII_VALUE']):?>
			<div class="wraps nomargin">
				<hr />
				...
				<?$APPLICATION->IncludeComponent("bitrix:news.list", "front_akc", array(
					"IBLOCK_TYPE" => "aspro_scorp_content",
					"IBLOCK_ID" => CCache::$arIBlocks[SITE_ID]["aspro_next_content"]["aspro_next_stock"][0],
					"NEWS_COUNT" => "3",
					"SORT_BY1" => "ACTIVE_FROM",
					"SORT_ORDER1" => "DESC",
					"SORT_BY2" => "SORT",
