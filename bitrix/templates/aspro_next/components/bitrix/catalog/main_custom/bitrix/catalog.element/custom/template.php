...
<div class="tabs_section">
	...
	<div class="tabs">
		<ul class="nav nav-tabs">
			...
			<?endif;?>
			<?if($arParams["SHOW_ADDITIONAL_TAB1"] == "Y"):?>
				<li class="<?=(!($iTab++) ? ' active' : '')?>">
					<a href="#dops1" data-toggle="tab"><span><?=($arParams["TAB_DOPS1_NAME"] ? $arParams["TAB_DOPS1_NAME"] : GetMessage("ADDITIONAL_TAB"));?></span></a>
				</li>
			<?endif;?>
		</ul>
		<div class="tab-content">
															...
															<td class="char_value">
																<span itemprop="value">
																	<?if(count($arProp["DISPLAY_VALUE"]) > 1):?>
																		<?=implode(', ', $arProp["DISPLAY_VALUE"]);?>
																	<?else:?>
																	<? $str = mb_strtolower($arProp[CODE]); ?>
																		<a href="<?=$arResult['SECTION']['SECTION_PAGE_URL']?>filter/<?=$str;?>-is-<?=$arProp[VALUE_XML_ID];?>/apply/" target="_blank"><?=$arProp["DISPLAY_VALUE"];?></a>
																	<?endif;?>
																</span>
															</td>
														</tr>
			...
			<?endif;?>
			<?if($arParams["SHOW_ADDITIONAL_TAB1"] == "Y"):?>
				<div class="tab-pane additional_block<?=(!($iTab++) ? ' active' : '')?>" id="dops1">
					<div class="title-tab-heading visible-xs"><?=($arParams["TAB_DOPS1_NAME"] ? $arParams["TAB_DOPS1_NAME"] : GetMessage("ADDITIONAL_TAB"));?></div>
					<div>111
					<? 
					$APPLICATION->IncludeFile(SITE_DIR."include/additional_products_description.php", array(), array("MODE" => "html", "NAME" => GetMessage('CT_BCE_CATALOG_ADDITIONAL_DESCRIPTION')));?>
					</div>
