...
<script>
	...
	jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases = {
		'duration':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_DURATION")?>',
		'title':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_TITLE")?>',
		'description':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_DESCRIPTION")?>',
		'file':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_FILE")?>',
		'download':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_DOWNLOAD")?>',
		'size_mb':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_BXTV_SIZE_MB")?>',
		'play':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_BXTV_PLAY")?>',
		'edit':'<?=GetMessageJS("MIBOK_SP_BITRIXTV_TEMPLATE_BXTV_EDIT")?>',
                'size':'<?=GetMessageJS(MIBOK_SP_BITRIXTV_TEMPLATE_SIZE)?>'
	};
	...
	jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].AddPlayerListener(
		...
		{
			txt =
			'<div onmouseover="jsPublicTVCollector.add[<?=$arResult['PREFIX']?>].HoverListItem(this)" onmouseout="jsPublicTVCollector.add[<?=$arResult['PREFIX']?>].HoverListItem(this)" style="padding:22px">'				
				+'<div class="bitrix-tv-tree-item-description">'
					+'<h4 tabindex="0" class="tv-desc-name">' + jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['Name'] + '</h4>' //name
					+jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['Description'] //description
					+'<div class="delimiter-tv-param-line-middle">'
                                        +(jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['Duration'].length >0
                                                                ?''
                                                                        +'<div class="delimiter-tv-param-line">'
                                                                                +'<span class="tv-gray">'+jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases.duration+' '+jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['Duration']
                                                                                +'</span>'
                                                                                + (jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['Size'].length >0
                                                                                        ?' <span class="tv-gray">('+jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases.size+jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['Size']+jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases.size_mb+')</span>'
                                                                                        :'')
                                                                        +'</div>'
                                                                :'') //duration
						...
						+'<div class="delimiter-tv-param-line">'
							+'<a class="btn btn-default" href="' + jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>]['Sections'][i]['Items'][j]['File'] + '">'
								+ jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases.download + '</a>'									
						+'</div>'						
						+'<div class="delimiter-tv-param"></div>'
						+'<div class="delimiter-tv-param-line">'
							+ '<a class="btn btn-default" href="javascript:void(0)" onclick="jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].PlayFile('+i+','+j+',true,true)">' + jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases.play + '</span>'
						+'</div>'						
						+'<div style="clear:both;"></div>'
					+'</div>'
				+'</div>'
