...
<?// display properties?>
<?$arDisplayPropertiesCodes = array_diff(array_keys($arResult['DISPLAY_PROPERTIES']), array('PRICE', 'PRICEOLD', 'PERIOD', 'PHOTOS', 'DOCUMENTS', 'LINK_GOODS', 'LINK_STAFF', 'LINK_REVIEWS', 'LINK_PROJECTS', 'LINK_SERVICES', 'FORM_ORDER', 'FORM_QUESTION', 'PHOTOPOS'));?>
...
<?if($arResult['DISPLAY_PROPERTIES']['FORM_ORDER']['VALUE_XML_ID'] == 'YES'):?>
	<?$bPrice = strlen($arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']);?>
	<div class="order-block">
		<div class="row">
			<div class="valign <?=($bPrice ? 'col-md-6' : 'col-md-4 col-sm-4 col-xs-5')?>">
				<?// element price?>
				<?if(strlen($arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE'])):?>
					<div class="price <?=($arResult['DISPLAY_PROPERTIES']['PRICEOLD']['VALUE'] ? '' : 'nop')?>">
						<div class="price_new">
							<span class="price_val"><?=$arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']?></span>
						</div>
						<?if($arResult['DISPLAY_PROPERTIES']['PRICEOLD']['VALUE']):?>
							<div class="price_old">
								<span class="price_val"><?=$arResult['DISPLAY_PROPERTIES']['PRICEOLD']['VALUE']?></span>
							</div>
						<?endif;?>
					</div>
				<?endif;?>
				<span class="btn btn-default btn-lg" data-event="jqm" data-param-id="<?=CCache::$arIBlocks[SITE_ID]['aspro_scorp_form']['aspro_scorp_order_study'][0]?>" data-name="order_study" data-autoload-study="<?=$arResult['NAME']?>"><span><?=(strlen($arParams['S_ORDER_STUDY']) ? $arParams['S_ORDER_STUDY'] : GetMessage('S_ORDER_STUDY'))?></span></span>
			</div>
			<div class="valign <?=($bPrice ? 'col-md-6' : 'col-md-8 col-sm-8 col-xs-7')?>">
				<div class="text">
					<?$APPLICATION->IncludeComponent(
						'bitrix:main.include',
						'',
						Array(
							'AREA_FILE_SHOW' => 'file',
							'PATH' => SITE_DIR.'include/ask_study.php',
							'EDIT_TEMPLATE' => ''
						)
					);?>
				</div>
