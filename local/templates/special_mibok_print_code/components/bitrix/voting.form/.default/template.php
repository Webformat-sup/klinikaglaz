...
?>
    ...
    <?if(isset($arResult["CAPTCHA_CODE"])){?>
        <div class="vote-item-header">
            <div class="vote-item-title vote-item-question"><?=GetMessage("MIBOK_SP_F_CAPTCHA_TITLE")?></div>
            <div class="vote-clear-float"></div>
        </div>
        <div class="form-group">
            <input type="hidden" name="captcha_code" value="<?=$arResult["CAPTCHA_CODE"]?>"/>
            <img src="/bitrix/tools/captcha.php?captcha_code=<?=$arResult["CAPTCHA_CODE"]?>" alt="<?=GetMessage("MIBOK_SP_F_CAPTCHA_TITLE")?>" />
        </div>
        <div class="form-group">
            <label for="captcha_word"><?=GetMessage("MIBOK_SP_F_CAPTCHA_PROMT")?><span class='starrequired'>*</span></label><br />
            <input class="form-control" type="text" size="20" name="captcha_word" />
        </div>
    <?}?><br />
    <div class="btn-group">
        <input type="submit" class="btn btn-default" name="vote" value="<?=GetMessage("MIBOK_SP_VOTE_SUBMIT_BUTTON")?>" />
    </div><br /><br />
    <p><a name="show_result" href="<?=$arResult["URL"]["RESULT"]?>"><?=GetMessage("MIBOK_SP_VOTE_RESULTS")?></a></p>
</form>
