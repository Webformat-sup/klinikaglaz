	...
	if($('[name="captcha_sid"]').length > 0){
		$('[name="captcha_sid"]').each(function(){
			...
			if($img.length > 0){
				...
				$img.after('<a class="btn btn-default no-margin voice_captcha_btn" href="#" audio-index="-1" aria-label="Озвучить изображение"><span class="glyphicon glyphicon-play"></span></a>');
			}
		});
	}
});
